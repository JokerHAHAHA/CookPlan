...
Class Planning
{
    public static function makePlanning($meals, $duration)
    {
        ...
        for ($day=0; $day < $duration; $day++) {
            ...
            for ($menu=0; $menu < 2; $menu++) { 
                ...
                //APERITIF
                $targetAperitif = self::getRandomIndex(0, sizeof($aperitifs)-1, $used_id);
    ...
    public static function getRandomIndex($min, $max, $used_id)
    {
        $index = rand(0,sizeof($meats)-1);
        if(in_array($meats[$targetMeat]['id'], $used_id))
        {
